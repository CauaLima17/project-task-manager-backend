...
const TarefaRepositorio = {
    async findAll() {
        ...
        return rows.map(row => new Tarefa(row.id, row.titulo, row.descricao, row.projeto_id, row.user_id, row.STATUS, row.prioridade));
    },

    async createTask(tarefa) {
        const result = await DBInterface.query('INSERT INTO tarefa (titulo, descricao, projeto_id, user_id) VALUES (?, ?, ?, ?)', [tarefa.titulo, tarefa.descricao, tarefa.projeto_id, tarefa.user_id]);
